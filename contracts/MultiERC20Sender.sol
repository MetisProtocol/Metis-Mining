...
// File: contracts/external/MultiERC20Sender.sol
...
contract MultiERC20Sender {
    ...
    function distributeToken(
        ...
        onlyManager
    {
        require(_recipients.length == _amounts.length, "!equal length");

        for(uint i = 0; i < _recipients.length; i++){
            // Send token to _recipients
            _token.safeTransfer(_recipients[i], _amounts[i]);
            emit SendToken(_recipients[i], address(_token), _amounts[i]);
        }
    }
        ...
        onlyManager
    {
        ...
        for(uint i = 0; i < _recipients.length; i++){
            ...
            emit SendToken(_recipients[i], address(0), _amounts[i]);
        }
    }
    ...
    event SendToken(address indexed recipient, address indexed token, uint256 amount);
